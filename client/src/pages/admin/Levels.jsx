...
export default function Levels() {
  ...
  async function fetchPage(page = 1) {
    setLoading(true);
    try {
      const { data } = await api.get('/levels', { params: { page, limit: LIMIT } });
      setItems(data.levels);
  ...
  async function handleSave(e) {
    ...
    try {
      if (modal.item) {
        await api.put(`/levels/${modal.item.id}`, { name });
  ...
  async function handleDelete() {
    setSaving(true);
    try {
      await api.delete(`/levels/${modal.item.id}`);
  ...
  return (
    <AdminLayout>
        ...
        <div className="bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,101,101,0.04)] border border-outline-variant/10 overflow-hidden">
          {items.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <p className="text-on-surface-variant font-semibold mt-3">No levels yet</p>
      ...
      {modal?.type === 'form' && (
        <Modal title={modal.item ? 'Edit Level' : 'Add Level'} onClose={() => setModal(null)}>
      ...
      {modal?.type === 'delete' && (
        <Modal title="Delete Level" onClose={() => setModal(null)}>
